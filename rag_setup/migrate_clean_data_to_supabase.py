"""
Migrate Clean Excel Data to Supabase
====================================
Este script migra los datos limpios del Excel a Supabase.
Usa el dataset ya procesado y limpio (rappi_data_clean_final.xlsx).
"""
import os
import sys
...
def read_clean_excel_data(file_path: str) -> pd.DataFrame:
    """Lee el archivo Excel limpio"""
    print(f"📂 Reading clean Excel file: {file_path}")
    ...
    for path in paths_to_try:
        if Path(path).exists():
            print(f"✅ Found clean file at: {path}")
            df = pd.read_excel(path)
            print(f"   Shape: {df.shape}")
            print(f"   Columns: {list(df.columns)}")
            return df

    raise FileNotFoundError(f"❌ Could not find clean Excel file. Tried: {paths_to_try}")
...
def create_supabase_tables():
    ...
    CREATE TABLE IF NOT EXISTS metrics_input (
        ...
        zone_prioritization VARCHAR(50),
        metric VARCHAR(100),
    ...
    CREATE INDEX IF NOT EXISTS idx_metrics_input_zone_type ON metrics_input(zone_type);
    CREATE INDEX IF NOT EXISTS idx_metrics_input_zone_prioritization ON metrics_input(zone_prioritization);
    """
...
def prepare_clean_metrics_data(df: pd.DataFrame) -> List[Dict]:
    """Prepara datos limpios para la tabla metrics_input - compatible con formato RAW_INPUT_METRICS"""
    print("\n📊 Preparing clean metrics_input data...")
...
def prepare_clean_orders_data(df: pd.DataFrame) -> List[Dict]:
    """Prepara datos limpios para la tabla orders - compatible con formato RAW_ORDERS"""
    print("\n📦 Preparing clean orders data...")
...
def upload_to_supabase(table_name: str, data: List[Dict], batch_size: int = 100, use_upsert: bool = True):
    ...
    for i in range(0, len(data), batch_size):
        ...
        try:
            if use_upsert:
                # UPSERT - reemplaza registros duplicados
...
def verify_upload():
    ...
    try:
        ...
        print("\n   Sample from orders:")
        sample_orders = supabase.table('orders').select('*').limit(3).execute()
        for row in sample_orders.data:
            print(f"      {row.get('country')} - {row.get('zone')}: L0W={row.get('l0w')}, L1W={row.get('l1w')}")

        # Estadísticas por métrica
        print("\n   Records by metric:")
        metrics_stats = supabase.table('metrics_input').select('metric', count='exact').execute()
        # Agrupar por métrica
        metric_counts = {}
        for row in metrics_stats.data:
...
def main():
    ...
    print("=" * 70)
    print("Migrate Clean Excel Data to Supabase")
    print("=" * 70)
    ...
    # Leer Excel limpio - Sheet RAW_INPUT_METRICS
    try:
        print(f"\n📂 Reading clean data from: {CLEAN_EXCEL_FILE}")
        df_clean = read_clean_excel_data(CLEAN_EXCEL_FILE)
        ...
        # Mostrar distribución de métricas
        ...
        print(f"❌ Error reading clean Excel file: {e}")
        return

    # Leer Excel - Sheet RAW_ORDERS
    try:
        print("\n📂 Reading sheet 'RAW_ORDERS'...")
        excel_path = Path(__file__).parent.parent / "data" / "raw" / "rappi_data.xlsx"
    ...
    print("\n⚠️  This will upload clean data to Supabase. Continue? (y/n)")
    response = input().strip().lower()
    if response != 'y':
        print("❌ Cancelled by user")
        return

    # Opción para limpiar tabla antes de insertar
    ...
    metrics_data = prepare_clean_metrics_data(df_clean)
    if metrics_data:
    ...
    if df_orders is not None:
        orders_data = prepare_clean_orders_data(df_orders)
        if orders_data:
    ...
    print("\n" + "=" * 70)
    print("Clean data migration complete!")
    print("=" * 70)
